...
def toroidDistance(myarray,width,height):
    somDim2 = []
    allcombs = itertools.combinations(myarray,2)
    for x,nx in allcombs:
...
def wordnetClusterSimilarityComputer(clusterTerms):
    ...
    for tt in allcombs:
        ...
        ss2 = wn.synsets(tt[1])
        try:
            maxSimilarities.append(max([x for x in [s1.wup_similarity(s2) for (s1, s2) in itertools.product(ss1, ss2)] if x]))
        except:
            pass
...
for lIdx,lvl in enumerate(LVLs):
    ...
    for idy,years in enumerate(yearPeriods):
        ...
        for filename in files:
            ...
            # interactive(True)
            # plt.show()            
            # fig.savefig(greenwichFigWritePath+'/'+clusterAlgLabel+'Clusters/'+SOMdimensionsString+'/SOM_AdjMat'+years+lvl+'_'+periodIdx+'.png',bbox_inches='tight')
            # plt.close()
            # interactive(False)
            ...
            # interactive(True)
            # plt.show()            
            # fig.savefig(greenwichFigWritePath+'/'+clusterAlgLabel+'Clusters/'+SOMdimensionsString+'/SOM_Wmatrix'+labelFormat+'LabeledAdjMat'+years+lvl+'_'+periodIdx+'.png',bbox_inches='tight')
            # plt.close()
            # interactive(False)
